...
**Every health indicator in this file MUST map to a RESULT in RESULTS_{name}.md.**
**Every RESULT must have a health indicator verifying its sense is running.**
**A health indicator without a result is noise. A result without a health indicator is unverified.**

```
RESULT (what success looks like)     ←── RESULTS file
  ↕ proved_by
SENSE (continuous measurement)       ←── SENSES file / sense.yaml
  ↕ verified_by_health
HEALTH (this file — verifies sense)  ←── YOU ARE HERE
  ↕ carried_by
CARRIER (citizen who FEELS it)       ←── assigned citizen
```

**This file is the runtime half of the guarantee loop.** RESULTS defines what must be true. SENSES measures it. HEALTH verifies the measurement is actually happening. If a sense silently stops firing, this file's checkers catch it and the carrier FEELS the gap in their awareness.
...
| **Sense verification** | **Ensures senses are actually firing and proving results** |

**Tests gate completion. Health monitors runtime. Results define success. Senses measure it. Health guarantees the measurement.**
...
If a result must be continuously proven → write a sense AND a health check.
...
## RESULTS COVERAGE (MANDATORY)

**Every result in RESULTS_{name}.md must appear in this table with a health indicator.**

| Result ID | Result Name | Sense (proved_by) | Health Indicator | Status |
|-----------|-------------|-------------------|------------------|--------|
| R1 | {result name} | sense:{scope}:{slug} | {indicator_name} | {active/pending} |

**If any result has no health indicator, the module is incomplete. Add it before shipping.**
