...
```
RESULTS → OBJECTIVES → PATTERNS → VOCABULARY → BEHAVIORS → ALGORITHM → VALIDATION → IMPLEMENTATION → HEALTH → SYNC
```
...
### The Guarantee Loop: RESULTS ↔ SENSES ↔ HEALTH (MANDATORY)

**A module is NOT complete unless every RESULT is continuously measured and felt by a citizen.**

This is not optional. This is the mechanism that guarantees the system works or we know IMMEDIATELY when it doesn't. The loop:

```
RESULT (what success looks like)
  ↕ 1-1
SENSE (continuous measurement — modulates objective node weight)
  ↕ 1-1
HEALTH (runtime verification that the sense is actually firing)
  ↕
CARRIER (citizen who FEELS the objective — bright when healthy, dim when failing)
  ↕
ACTION (citizen acts when they feel degradation)
```

**Rules:**
1. **Every RESULT must have at least one SENSE** in its `proved_by:` field. A result without a sense is a promise nobody is keeping.
2. **Every SENSE must trace to a RESULT** via its `proves_result:` field. A sense without a result is measurement without purpose.
3. **Every HEALTH indicator must map to a RESULT** it protects. A health check without a result is noise.
4. **The loop integrity checker runs on every sense tick.** If any link (sense → objective → result) is missing, a task is auto-created for the carrier with energy 0.8 (high enough to enter WM). The citizen FEELS the broken loop.
5. **Orphan results are caught by the Result Loop Guardian** (every 50 ticks). A result with no senses feeding it gets flagged automatically.

**Module completeness checklist:**
- [ ] RESULTS file exists with concrete, measurable outcomes
- [ ] Every result has `proved_by: [sense:scope:slug]` pointing to real senses
- [ ] SENSES file exists with measurement logic for each sense
- [ ] Every sense has `proves_result: R1` tracing back to results
- [ ] HEALTH file exists documenting runtime verification of each sense
- [ ] Every health indicator maps to a VALIDATION criterion
- [ ] Carriers assigned — a citizen FEELS each signal
- [ ] Wiring verified — objective nodes exist in L3 with CONTRIBUTES_TO links
