...
def scan_and_ingest_files(
    ...
    Args:
        ...
        embed: Generate embeddings for nodes (default: False)
    ...
    # Embed if requested
    if embed and (stats["things_created"] > 0 or stats["things_updated"] > 0):
        _embed_things(adapter)
...
def _embed_things(adapter) -> int:
    """Embed all Thing nodes that don't have embeddings."""
    try:
        from ..infrastructure.embeddings.service import EmbeddingService

        embed_service = EmbeddingService()

        # Find Things without embeddings
        result = adapter.query(
            """
            MATCH (t:Thing)
            WHERE t.embedding IS NULL AND t.synthesis IS NOT NULL
        ...
        count = 0
        for row in result:
            thing_id = row.get('t.id') or row[0]
            synthesis = row.get('t.synthesis') or row[1]

            embedding = embed_service.embed(synthesis)
            if embedding:
                adapter.execute(
                    "MATCH (t:Thing {id: $id}) SET t.embedding = $embedding",
                    {"id": thing_id, "embedding": embedding}
                )
                count += 1
