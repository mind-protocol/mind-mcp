...
def _sanitize_windows_path(path_str):
    """Validate and sanitize a Windows path to prevent PowerShell injection."""
    windows_path = str(path_str).replace('/mnt/c', 'C:').replace('/', '\\')
    # Only allow alphanumeric, backslash, colon, dot, dash, underscore, space
    if not re.match(r'^[A-Za-z]:\\[\w\s.\-\\]+$', windows_path):
        raise ValueError(f"Invalid Windows path (possible injection): {windows_path}")
    return windows_path
...
def capture_screen_windows(output_path):
    ...
    windows_path = _sanitize_windows_path(output_path)

    # PowerShell script uses $args[0] parameter instead of f-string interpolation
    ps_script = '''
    ...
    # Write script to temp file, pass path as argument (not interpolated)
    temp_ps1 = "/tmp/capture_screen.ps1"
    ...
    try:
        result = subprocess.run(
            ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass',
             '-File', temp_ps1, '-savePath', windows_path],
            capture_output=True,
            text=True
        )
...
def capture_screen_snippingtool(output_path):
    ...
    # Script uses param() — path is passed as argument, not interpolated
    ps_script = '''
    param([string]$savePath)
