...
| **Repair, Sync, Validate, etc.** | General notes appear in CLI core or in their respective command directories if they get their own PATTERNS/…/SYNC chain. | Refer to `docs/cli/core/` for shared guidance, plus the per-module VIEW files under `.mind-mcp/views/`. |
